...
open class EventSource: NSObject, EventSourceProtocol, URLSessionDataDelegate {
    ...
    public func connect(lastEventId: String? = nil) {
        connect(lastEventId: lastEventId, reqString: nil)

    }

    public func connect(lastEventId: String? = nil, reqString: String? = nil) {
        eventStreamParser = EventStreamParser()
        readyState = .connecting

        let configuration = sessionConfiguration(lastEventId: lastEventId)
        urlSession = URLSession(configuration: configuration, delegate: self, delegateQueue: operationQueue)
        var myReq = URLRequest(url: url)
        let userAgent =     "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:109.0) Gecko/20100101 Firefox/114.0";
        myReq.setValue(userAgent, forHTTPHeaderField: "User-Agent")

        myReq.httpMethod = "POST"
        myReq.httpBody = reqString?.data(using: .utf8)!
        urlSession?.dataTask(with: myReq).resume()
    }
    ...
    open func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        logD("received stringData = \(stringData)")
...
internal extension EventSource {
    ...
    func sessionConfiguration(lastEventId: String?) -> URLSessionConfiguration {
        ...
        additionalHeaders["Authorization"] =  "Bearer \(SettingsViewModel.shared.accessToken)"
