...
public final class MultipeerTransceiver {
    private let multipeerQueue = DispatchQueue(label: "com.swift-sage.multipeer2", qos: .userInitiated)
...
//    }
    ...
    init(configuration: MultipeerConfiguration, modes: [TransmitterMode]) {
    ...
    private func configure(_ connection: MultipeerProtocol) {
        connection.didReceiveData = { data, peer in
            self.handleDataReceived(data, from: peer)
        }
        connection.didFindPeer = {  peer in
            //self?.multipeerQueue.async {
                self.handlePeerAdded(peer)
           // }

        }
        connection.didLosePeer = {  peer in
            //self?.multipeerQueue.async {

            self.handlePeerRemoved(peer)

          // }
        }
        connection.didConnectToPeer = {  peer in
            //self?.multipeerQueue.async {
            self.handlePeerConnected(peer)

          // }
        }
        connection.didDisconnectFromPeer = {  peer in
           // self?.multipeerQueue.async {
