...
//        fprintf(stderr, "whisperSize= %7.2f MB\n", ctx_size/(1024.0*1024.0));
    }
                        ...
                        if (!text.empty()) {
                            ...
                            if (params.print_realtime) {
                                if (params.print_timestamps) {
                                    printf("WHISPER FOUND UTTERANCE: [%s --> %s]  %s\n", to_timestamp(tt0).c_str(), to_timestamp(tt1).c_str(), text.c_str());

                                    // How can I reach out and call gptPrompt from here?
...
int whisper_full_parallel(
    ...
    for (int i = 0; i < n_processors - 1; ++i) {
        ...
        params_cur.print_progress = true;
        params_cur.print_realtime = true;
    ...
    // fprintf(stderr, "\n");
    // printf(stderr, "%s: the audio has been split into %d chunks at the following times:\n", __func__, n_processors);
    for (int i = 0; i < n_processors - 1; ++i) {
